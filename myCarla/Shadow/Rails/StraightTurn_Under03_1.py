...
def parse_arg():
    ...
    argparser.add_argument(
        '--output', '-o',
        type=str,
        default='/media/wanglu/MyPassport/Carla-Images/lane-detection-benchmark/Shadow/Rails/StraightTurn_Under03_1')
    argparser.add_argument(
        '--img_num', '-n',
        type=int,
        default=150)
...
def main():
    ...
    try:
        ...
        ts_start_point = carla.Transform(carla.Location(x=-145.503876, y=-16.109726, z=0.404872), carla.Rotation(pitch=0.000000, yaw=-89.970146, roll=0.000000))
        ...
        ts_end_point = carla.Transform(carla.Location(x=-67.154907, y=-136.210205, z=0.275307), carla.Rotation(pitch=0.000000, yaw=1.227276, roll=0.000000))
        print(ts_end_point)
        ...
        with CarlaSyncMode(world, camera_rgb, fps=10) as sync_mode:
            ...
            while True:
                ...
                # 处理数据
                if (i >= 7) and (i <= 106):
                    image_rgb.save_to_disk(os.path.join(save_dir, '{:06d}.png'.format(i - 7))) # image_rgb.frame
...
if __name__ == '__main__':
    ...
    for alt in alts:
        for azm in azms:
            ...
            try:
                os.system('python3 ../util/environment.py -alt {} -azm {}'.format(alt, azm))    # 阴影在正下方
            except Exception:
    ...
    try:
        os.system('python3 ../util/environment.py -alt 55 -azm 30')
    except Exception:
        print('eeeeeeeeeeeeeeeeee----')
        exit()
    try:
