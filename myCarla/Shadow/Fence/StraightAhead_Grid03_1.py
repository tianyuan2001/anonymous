...
def parse_arg():
    ...
    argparser.add_argument(
        '--output', '-o',
        type=str,
        default='/media/wanglu/MyPassport/Carla-Images/lane-detection-benchmark/Shadow/Fence/StraightAhead_Grid03_1')
    argparser.add_argument(
        '--img_num', '-n',
        type=int,
        default=49)
...
def main():
    ...
    # client.load_world('Town03_fence_grid')
    ...
    try:
        ...
        ts_start_point = carla.Transform(carla.Location(x=22.490116, y=207.505676, z=0.275307), carla.Rotation(pitch=0.000000, yaw=-0.142975, roll=0.000000))
        ...
        ts_end_point = carla.Transform(carla.Location(x=242.702316, y=129.078125, z=1.352807), carla.Rotation(pitch=0.000000, yaw=-88.606743, roll=0.000000))
        print(ts_end_point)
...
#################################################################### 12 348
    alts = [9, 12]
    azms = [344, 346, 348, 350, 352]
    for alt in alts:
        for azm in azms:
            save_dir = args.output + "_alt{}azm{}".format(alt, azm)
            os.system("python3 ../util/environment.py -alt {} -azm {}".format(alt, azm))    
            try:
                main()
...
####################################################################
    ...
    os.system('python3 ../util/environment.py -alt 12 -azm 0')
    try:
        main()
    except KeyboardInterrupt:
        print('\nCancelled by user. Bye!')
