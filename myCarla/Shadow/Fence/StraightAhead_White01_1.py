...
def parse_arg():
    ...
    argparser.add_argument(
        '--output', '-o',
        type=str,
        default='/media/wanglu/MyPassport/Carla-Images/lane-detection-benchmark/Shadow/Fence/StraightAhead_White01_1')
    argparser.add_argument(
        '--img_num', '-n',
        type=int,
        default=49)
...
def main():
    ...
    try:
        ...
        ts_start_point = carla.Transform(carla.Location(x=176.589493, y=133.239151, z=0.300000), carla.Rotation(pitch=0.000000, yaw=-0.000092, roll=0.000000))
...
if __name__ == '__main__':
    ...
    # 噪声场景   
    alts = [7, 9]
    azms = [320, 325, 330, 335]
    for alt in alts:
        for azm in azms:
            save_dir = args.output + "_alt{}azm{}".format(alt, azm)
            os.system('python3 ../util/environment.py --weather clear -alt {} -azm {}'.format(alt, azm)) 
            try:
                main()
    ...
    os.system('python3 ../util/environment.py --weather clear -alt 16 -azm 0')    
    try:
        main()
    except KeyboardInterrupt:
        print('\nCancelled by user. Bye!')
