...
def parse_arg():
    ...
    argparser.add_argument(
        '--output', '-o',
        type=str,
        default='/media/wanglu/MyPassport/Carla-Images/lane-detection-benchmark/Shadow/StreetLight/Turning_HighwayMultiple05_1')
    argparser.add_argument(
        '--img_num', '-n',
        type=int,
        default=300)
    ...
    try:
        ...
        with CarlaSyncMode(world, camera_rgb, fps=10) as sync_mode:
            ...
            while True:
                ...
                # 处理数据
                if (i >= 130) and (i <= 229):
                    image_rgb.save_to_disk(os.path.join(save_dir, '{:06d}.png'.format(i - 130))) # image_rgb.frame
                if i == 130:
...
if __name__ == '__main__':
    ...
    try:
        os.system('python3 ../util/environment.py -alt 18 -azm 120')
    except Exception:
        print('eeeeeeeeeeeeeeeeee----')
        exit()
    try:
