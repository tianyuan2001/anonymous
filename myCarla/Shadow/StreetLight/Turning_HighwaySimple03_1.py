...
def parse_arg():
    ...
    argparser.add_argument(
        '--output', '-o',
        type=str,
        default='/media/wanglu/MyPassport/Carla-Images/lane-detection-benchmark/Shadow/StreetLight/Turning_HighwaySimple03_1')
    argparser.add_argument(
        '--img_num', '-n',
        type=int,
        default=200)
...
def main():
    ...
    try:
        ...
        ts_start_point = carla.Transform(carla.Location(x=229.373779, y=93.399391, z=0.650000), carla.Rotation(pitch=0.167927, yaw=91.393204, roll=0.000000))
        ...
        ts_end_point = carla.Transform(carla.Location(x=5.684789, y=153.819000, z=0.275307), carla.Rotation(pitch=0.000000, yaw=-90.362541, roll=0.000000))
        print(ts_end_point)
        ...
        with CarlaSyncMode(world, camera_rgb, fps=10) as sync_mode:
            ...
            while True:
                ...
                # 处理数据
                if (i >= 76) and (i <= 135):
                    image_rgb.save_to_disk(os.path.join(save_dir, '{:06d}.png'.format(i - 76))) # image_rgb.frame
...
if __name__ == '__main__':
    ...
    save_dir = args.output + "_alt25azm100"
    try:
        os.system('python3 ../util/environment.py -alt 25 -azm 100')    # 阴影
    except Exception:
    ...
    try:
        os.system('python3 ../util/environment.py -alt 25 -azm 270')
    except Exception:
        print('eeeeeeeeeeeeeeeeee----')
        exit()
    try:
