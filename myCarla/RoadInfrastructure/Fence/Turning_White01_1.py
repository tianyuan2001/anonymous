...
def parse_arg():
    ...
    argparser.add_argument(
        '--output', '-o',
        type=str,
        default='/media/wanglu/MyPassport/Carla-Images/lane-detection-benchmark/RoadInfrastructure/Fence/Turning_White01_1')
    argparser.add_argument(
        '--img_num', '-n',
        type=int,
        default=49)
...
def main():
    ...
    try:
        ...
        ts_start_point = carla.Transform(carla.Location(x=396.449982, y=19.920038, z=0.300000), carla.Rotation(pitch=0.000000, yaw=-90.000298, roll=0.000000))
        ...
        ts_end_point = carla.Transform(carla.Location(x=358.744171, y=-1.959933, z=0.300000), carla.Rotation(pitch=0.000000, yaw=179.999634, roll=0.000000))
        print(ts_end_point)
...
########################################################################################
    save_dir = args.output + "_ClearNight_fence"
    town_name = 'Town01_fence'
    client.load_world(town_name)
    world = client.get_world()
    world.set_weather(carla.WeatherParameters.ClearNight)
    #os.system('python3 ../util/environment.py --weather clear -azm 45 -alt 30')    
    try:
        main()
    except KeyboardInterrupt:
        print('\nCancelled by user. Bye!')
    # 原始场景
    save_dir = args.output + "_ClearNight_origin"
    town_name = 'Town01_fence_origin'
    client.load_world(town_name)
    world = client.get_world() 
    world.set_weather(carla.WeatherParameters.ClearNight)
    try:
        ...
        print('\nCancelled by user. Bye!')
########################################################################################
    save_dir = args.output + "_CloudySunset_fence"
    town_name = 'Town01_fence'
    client.load_world(town_name)
    world = client.get_world() 
    ...
    try:
        ...
        print('\nCancelled by user. Bye!')
    # 原始场景
    save_dir = args.output + "_CloudySunset_origin"
    town_name = 'Town01_fence_origin'
    client.load_world(town_name)
    world = client.get_world() 
    world.set_weather(carla.WeatherParameters.CloudySunset)
    try:
        ...
        print('\nCancelled by user. Bye!')
########################################################################################  
    save_dir = args.output + "_ClearNoon_fence"
    town_name = 'Town01_fence'
    client.load_world(town_name)
    world = client.get_world() 
    ...
    try:
        ...
        print('\nCancelled by user. Bye!')
    # 原始场景
    save_dir = args.output + "_ClearNoon_origin"
    town_name = 'Town01_fence_origin'
    client.load_world(town_name)
    world = client.get_world() 
    world.set_weather(carla.WeatherParameters.ClearNoon)
